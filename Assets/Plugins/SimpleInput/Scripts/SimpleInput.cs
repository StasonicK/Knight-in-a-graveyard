...
public class SimpleInput : MonoBehaviour
{
    ...
    public class MouseButtonInput : BaseInput<int, bool>
    {
        ...
        protected override void RegisterInput()
        {
            RegisterMouseButton(this);
        }

        protected override void UnregisterInput()
        {
            UnregisterMouseButton(this);
        }
    }
    ...
    public class KeyInput : BaseInput<KeyCode, bool>
    {
        ...
        protected override bool KeysEqual(KeyCode key1, KeyCode key2)
        {
            return key1 == key2;
        }
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < axesList.Count; i++)
        {
			...
			if( axis.value >= 0 )
				axis.value =
 Mathf.MoveTowards( axis.valueRaw >= 0 || !GetAxisSnapValue ? axis.value : 0, axis.valueRaw, sensitivity );
			else
				axis.value =
 Mathf.MoveTowards( axis.valueRaw <= 0 || !GetAxisSnapValue ? axis.value : 0, axis.valueRaw, sensitivity );
#else
            if (axis.value >= 0)
                axis.value = Mathf.Lerp(axis.valueRaw >= 0 || !GetAxisSnapValue ? axis.value : 0, axis.valueRaw,
                    sensitivity);
            else
                axis.value = Mathf.Lerp(axis.valueRaw <= 0 || !GetAxisSnapValue ? axis.value : 0, axis.valueRaw,
                    sensitivity);
